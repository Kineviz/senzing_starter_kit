...
class json2attribute:
    ...
    def parse(self, json_string):
            ...
            if isinstance(json_data[attribute], list):
                ...
                for child_data in json_data[attribute]:
                    ...
                    for record_attribute in (x for x in child_data if child_data[x]):
                        attr_data = self.lookup_attribute(
                            record_attribute.upper(), child_data[record_attribute]
                        )
                        segment_id = f"{attribute}-{i}"
                        self.update_grouping(segment_id, attr_data)
            else:
                attr_data = self.lookup_attribute(
                    attribute.upper(), json_data[attribute]
                )
                segment_id = "ROOT"
                self.update_grouping(segment_id, attr_data)
        ...
        for segment_id in self.attr_groups:
            ...
            for attr_data in sorted(
                self.attr_groups[segment_id], key=itemgetter("ATTR_ID")
            ):
                ...
                if attr_data.get("FELEM_CODE") == "USAGE_TYPE":
                    ...
                    attr_data.get("FELEM_CODE") == "KEY_TYPE"
                    and attribute == "REL_POINTER"
                ):
    ...
    def update_grouping(self, segment_id, attr_data):
        segment_id += (
            f"|{attr_data.get('FTYPE_CODE','')}|{attr_data.get('USAGE_TYPE','')}"
        )
...
def export_nodes(senzing_graph, source_graph, entity_id, record_list):
        ...
        for attr_data in sorted(attr_list, key=itemgetter("ATTR_ID")):
            ...
            if attr_data["ATTRIBUTE"] == "REL_POINTER":
                if args.include_source_nodes:
                    ...
                    role = (
                        attr_data["USAGE_TYPE"]
                        if attr_data["USAGE_TYPE"]
                        else "RELATED_TO"
                    )
                    link = {
                    ...
                    if sz_dbo.fetchNext(
                        sz_dbo.sqlExec(
                            "select 1 from DSRC_RECORD where DSRC_ID = ? and RECORD_ID = ?",
                            (row["DSRC_ID"], related_record_id),
                        )
                    ):
                ...
                if attr_data["ATTRIBUTE"] == "NAME":
                    if attr_data.get("USAGE_TYPE") == "PRIMARY":
                        ...
                        longest_primary_name = (
                            ...
                            if len(attr_data["ATTR_VALUE"]) > len(longest_primary_name)
                            else longest_primary_name
                        )
                    else:
                        longest_other_name = (
                            attr_data["ATTR_VALUE"]
                            if len(attr_data["ATTR_VALUE"]) > len(longest_other_name)
                            else longest_other_name
                        )
                        if not record_name:
                            record_name = attr_data["ATTR_VALUE"]

                attribute = (
                    f"{attr_data['USAGE_TYPE']}_{attr_data['ATTRIBUTE']}"
                    if attr_data.get("USAGE_TYPE")
                    else attr_data["ATTRIBUTE"]
                ).lower()
    ...
    node = {
        ...
        "node_name": (
            longest_primary_name if longest_primary_name else longest_other_name
        ),
        "entity_id": entity_id,
...
def dbo_export(senzing_graph, source_graph):
    ...
    max_entity_id = sz_dbo.fetchNext(
        sz_dbo.sqlExec("select max(RES_ENT_ID) as MAX_ENTITY_ID from RES_ENT")
    )["MAX_ENTITY_ID"]

    while True:
        logging.info(f"getting entities from {beg_entity_id} to {end_entity_id} ...")
        record_cursor = sz_dbo.fetchAllDicts(
            sz_dbo.sqlExec(get_records_sql, (beg_entity_id, end_entity_id))
        )
        ...
        for entity_records in groupby(record_cursor, key=itemgetter("RES_ENT_ID")):
            ...
            if entity_count % 100000 == 0:
                ...
                logging.info(
                    f"{entity_count:,} entities processed after {elapsed} minutes"
                )

            export_nodes(
                senzing_graph, source_graph, entity_records[0], list(entity_records[1])
            )
    ...
    while record:
        ...
        if relation_count % 100000 == 0:
            ...
            logging.info(
                f"{relation_count:,} relationships processed after {elapsed} minutes"
            )

        if (
            len(record["MATCH_LEVELS"].split(",")) > 1
        ):  # could 2 or 3 and 11 (we ignoring 11's disclosed relationships)
            record["MATCH_LEVELS"] = record["MATCH_LEVELS"].split(",")[0]
        link = {
            "source": f"SENZING:{record['ENTITY_ID']}",
            "target": f"SENZING:{record['RELATED_ENTITY_ID']}",
            "edge_class": "Derived",
            "edge_type": {"2": "Possible Match", "3": "Possibly Related"}.get(
                record["MATCH_LEVELS"], "Other"
            ),
            "edge_details": remove_drs(record["MATCH_KEY"]),
        }
        senzing_graph["links"].append(link)
        link["source"] = (
            f"SENZING:{record['RELATED_ENTITY_ID']}"  # must point to each other to be bi-directional
        )
    ...
    elapsed = round((time.time() - relationship_start_time) / 60, 1)
    logging.info(
        f"{relation_count:,} relationships processed after {elapsed} minutes, done!"
    )
...
if __name__ == "__main__":
    ...
    argParser.add_argument(
        "-o",
        "--output_path",
        help="Path to desired output files including base file name if desired as multiple files are created",
    )
    # argParser.add_argument('-f', '--output_format', default='G', help='(G)raphml or (C)SV format, default is Graphml')
    argParser.add_argument(
        "-S",
        "--include_source_nodes",
        action="store_true",
        default=False,
        help="Only resolved entity nodes and relationships are exported by default",
    )
    argParser.add_argument(
        "-D", "--debug", action="store_true", default=False, help="turn debug mode on"
    )
    ...
    if os.getenv("SENZING_ENGINE_CONFIGURATION_JSON"):
        ...
        try:
            ...
            sz_config = iniParamCreator.getJsonINIParams(
                os.getenv("SENZING_CONFIG_FILE")
            )
        except Exception as ex:
            logging.error(ex)
            sys.exit(1)
    ...
    source_graph = None
    if (
        args.include_source_nodes
    ):  # source relationships are directed and there can be maultiple
        source_graph = {
            "directed": True,
            "multigraph": True,
            "graph": {},
    ...
    logging.info(
        f"processed completed in {round((time.time() - proc_start_time) / 60, 1)} minutes"
    )
    sys.exit(0)
